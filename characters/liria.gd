...
func _ready():
	...
	print("Liria, la Guardiana de la Palabra initialized")
...
func _on_body_entered(body):
	if body == player:
		...
		print("Player near Liria")

func _on_body_exited(body):
	if body == player:
		player_in_range = false
		$Label.visible = false
		print("Player left Liria")

func start_dialogue():
	if Dialogic.current_timeline != null:
		print("Another dialogue is active, skipping")
		return
	Dialogic.start("Liria_Welcome")
	Dialogic.timeline_ended.connect(_on_dialogue_ended, CONNECT_ONE_SHOT)
	has_spoken = true
	print("Starting Liria_Welcome dialogue")

func _on_dialogue_ended():
	print("Liria_Welcome dialogue ended")
